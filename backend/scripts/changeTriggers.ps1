...
try {
    ...
    $result = Invoke-Command -Session $session -ScriptBlock {
        param($taskName, $index, $startBoundary, $repetitionInterval, $repetitionDuration, $daysInterval, $weeksInterval, $daysOfWeek, $daysOfMonth, $months)
        ...
        try {
            ...
            if (($index -lt 0) -or ($index -ge $existingTriggers.Count)) {
                $resultObj.Success = $false
                $resultObj.Error = "Trigger index out of range"
                return $resultObj
            }
            ...
            # 替換指定索引的觸發器
            $updatedTriggers = @()
            for ($i = 0; $i -lt $existingTriggers.Count; $i++) {
                if ($i -eq $index) {
                    $updatedTriggers += $newTrigger
                } else {
                    $updatedTriggers += $existingTriggers[$i]
                }
            }
            ...
            $resultObj.Message = "Trigger updated successfully"
        } catch {
            $resultObj.Success = $false
        ...
        return $resultObj
    } -ArgumentList $TaskName, $Index, $StartBoundary, $RepetitionInterval, $RepetitionDuration, $DaysInterval, $WeeksInterval, $DaysOfWeek, $DaysOfMonth, $Months
